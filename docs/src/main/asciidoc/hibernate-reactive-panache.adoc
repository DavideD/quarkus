...
[NOTE]
====
With the introduction of the `quarkus-reactive-transactions` extension (currently in preview status), you can now use `jakarta.transaction.Transactional` for transaction management in Hibernate Reactive applications.

However, you should not mix `@Transactional` with Panache transaction annotations (`@WithTransaction`, `@WithSession`, `@WithSessionOnDemand`) in the same application. Choose one approach and use it consistently throughout your application. Mixing these annotations will result in an `UnsupportedOperationException`.
